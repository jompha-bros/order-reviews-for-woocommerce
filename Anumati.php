...
class Anumati
{
    public  $key                     = '914CFCCB72733353';
    private $product_id              = '3';
    private $product_base            = 'order-reviews-for-woocommerce';
    private $server_host             = 'https://jompha.com/wp-json/jsl/';
    ...
    public function __construct( $plugin_base_file = '' )
    {
        ...
        if ( $this->hasCheckUpdate )
        {
            ...
            {
                add_action( 'admin_post_order-reviews-for-woocommerce_fupc', function()
                {
                ...
                else
                {
                    ...
                    add_filter( 'plugin_row_meta', function( $links, $plugin_file )
                    {
                        if ( $plugin_file == plugin_basename( $this->pluginFile ) )
                            $links[] = ' <a class="edit coption" href="' . esc_url( admin_url( 'admin-post.php' ) . '?action=order-reviews-for-woocommerce_fupc' ) . '">' . esc_html__( 'Update Check', 'order-reviews-for-woocommerce' ) . '</a>';
    ...
    public function updateMessageCB($data, $response)
    {
        ...
        if ( isset($data->package) && empty($data->package) )
        {
            if ( empty($data->update_denied_type) )
                print '<br/><span style="display: block; border-top: 1px solid #ccc;padding-top: 5px; margin-top: 10px;">' . esc_html__( 'Please activate license or renew support period to get latest version.', 'order-reviews-for-woocommerce' ) . '</span>';
            
            elseif ( $data->update_denied_type == 'L' )
                print '<br><span style="display: block; border-top: 1px solid #ccc;padding-top: 5px; margin-top: 10px;">' .  esc_html__( 'Please activate license to get latest version.', 'order-reviews-for-woocommerce' ) . '</span>';

            elseif ( $data->update_denied_type == 'S' )
                print '<br><span style="display: block; border-top: 1px solid #ccc;padding-top: 5px; margin-top: 10px;">' . esc_html__( 'Please renew support period to get latest version', 'order-reviews-for-woocommerce' ) . '</span>';
        }
    }
    ...
    {
        return unserialize( $this->decrypt( $ciphertext ) );
    }
    ...
    private function processs_response($response)
    {
        ...
        if ( ! empty( $response ) )
        {
            ...
            else
            {
                ...
                $response->msg    = esc_html__( 'Response Error, contact with the author or update the plugin', 'order-reviews-for-woocommerce' );
        ...
        $response         = new \stdClass();
        $response->msg    = esc_html__( 'Unknown Response', 'order-reviews-for-woocommerce' );
        $response->status = false;
        $response->data   = NULL;
    ...
    private function _request( $relative_url, $data, &$error = '' )
    {
        $response                   = new \stdClass();
        $response->status           = false;
        $response->msg              = esc_html__( 'Empty Response', 'order-reviews-for-woocommerce' );
        ...
        {
            $response->msg              = esc_html__( 'Curl extension is missing', 'order-reviews-for-woocommerce' );
            $response->status           = false;
            $response->data             = NULL;
            $response->is_request_error = true;
            return $response;
            ...
            if ( empty( $response->code ) )
            {
                if ( ! empty( $response->status ) )
                {
                    if ( ! empty( $response->data ) )
                    {
                        ...
                        $error = esc_html__( 'Invalid data', 'order-reviews-for-woocommerce' );
                }
                else
                    $error = $response->msg;
            }
